...
Status: **CANONICAL ARCHITECTURE / INVARIANTS — CURRENT PROJECT STATE IS OWNED BY `docs/CURRENT_STATE.md`**

## Foundation completeness constraints
...
- future internal-Flash ownership is frozen by `docs/FLASH_OWNERSHIP_LAYOUT_DECISION.md`: steady-state bootloader/recovery pages 0..7 (`0x08000000..0x08001FFF`), relocated application pages 8..61 from `0x08002000` with 54 KiB maximum, and persistent A/B pages 62/63; realization is phased so Asset/Configuration first keeps the application as standalone reset owner at `0x08000000` under the same 54 KiB size ceiling while pages 54..61 stay unused relocation headroom, and only the later Bootloader boundary changes reset/vector ownership and relocates the application;
...
## Accepted architecture — Deus OS product/application/UI model foundation
